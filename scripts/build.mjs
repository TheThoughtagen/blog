...
export function validateContent(site, notes, links) {
  ...
  for (const link of links) {
    if (!validWebUrl(link.url) || !link.title || !link.description || !['LinkedIn', 'Substack'].includes(link.source) || !categories.includes(link.category) || !dateIsValid(link.date)) throw new Error('Invalid external post.');
  }
}
...
<div class="statusbar"><div><button data-vim-toggle aria-pressed="true" aria-label="Toggle Vim navigation">-- NORMAL --</button><span class="status-file">${article ? e(article.slug) + '.md' : active + '.md'}</span></div><div class="key-hints"><span><kbd>j</kbd><kbd>k</kbd> navigate</span><button data-search><kbd>/</kbd> search</button><button data-help><kbd>?</kbd> keys</button></div><span class="status-position" data-scroll-position>TOP</span></div>
...
<dialog id="help-dialog" aria-labelledby="help-title"><div class="dialog-heading"><span id="help-title">A LITTLE LESS MOUSE</span><button data-close aria-label="Close keyboard help">esc</button></div><p class="help-intro">A familiar way to move around. All controls also work with a mouse or touch.</p><dl class="shortcut-list"><div><dt><kbd>j</kbd> / <kbd>k</kbd></dt><dd>Next / previous visible note</dd></div><div><dt><kbd>enter</kbd></dt><dd>Open the focused note</dd></div><div><dt><kbd>g</kbd><kbd>g</kbd> / <kbd>G</kbd></dt><dd>Top / bottom of page</dd></div><div><dt><kbd>/</kbd> or <kbd>Ctrl/Cmd K</kbd></dt><dd>Search the notebook</dd></div><div><dt><kbd>:</kbd></dt><dd>Open the command line</dd></div><div><dt><kbd>?</kbd> / <kbd>esc</kbd></dt><dd>Help / close a dialog</dd></div></dl><p class="help-intro">Commands: <code>:home</code>, <code>:lab</code>, <code>:about</code>, <code>:theme green</code>, <code>:theme amber</code>, <code>:theme paper</code>, <code>:reboot</code>. Turn off Vim navigation in the bottom-left status bar to disable single-key shortcuts. Ctrl/Cmd K always opens search.</p></dialog>
...
function home(site) {
  ...
  const body = `<div class="wrap"><div class="eyebrow-line"><span><i class="tiny-square"></i> PERSONAL ENGINEERING LOG</span><span>PATRICK MANNION / FIELDNOTES</span></div>
    ...
    <aside class="notebook-sidebar"><div class="sidebar-heading"><span class="signal-dot"></span> FROM THE WORKBENCH</div>${githubPanel(site, 'activity')}<a class="sidebar-lab-link" href="/lab/">Releases &amp; experiments <span>&#8599;</span></a><div class="sidebar-note"><span class="overline">A QUESTION TO START WITH</span><p>Can the next person <em>diagnose it?</em></p><span>Name the symptom. Explain the next step.</span></div></aside></div>
    </section>
